/* ask user for loan amount, ask for apr, and how many years the loan duration is. 
for the monthly apr, apr must be divided by 12, and for loan duration in months, loan duration must
multiply by 12*/
...
console.log(`So, you requested a loan of $${loanAmount}, at an APR of ${percentage}%,\nover the course of ${duration} years.`)

let monthsToPay = Number(duration * 12);
let monthPercent = Number((percentage / 100)/ 12).toFixed(2);
let output = (loanAmount*(monthPercent/(1-(1+monthPercent)**(-monthsToPay))));
prompt(`Your monthly payment amount is $${output}, to be paid over ${monthsToPay} months!`);
